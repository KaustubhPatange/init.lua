...
function M.provider.capslock(opts)
  opts = extend_tbl({ str = "CAPSLOCK", padding = { right = 1, left = 1 }, show_empty = true }, opts)
  return function() return M.utils.stylize(opts.str, opts) end
end
...
function M.component.capslock(opts)
  opts = extend_tbl({
    capslock = { str = "CAPSLOCK" },
    surround = { separator = "right", color = M.env.fallback_colors.red },
    hl = { fg = M.env.fallback_colors.dark_bg, bold = true },
    condition = function()
      local p = require "capslock"
      return p.status_string == p.status_string_const
    end,
  }, opts)
  return M.component.builder(M.utils.setup_providers(opts, { "capslock" }))
end
